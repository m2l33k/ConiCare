...
export default function ScheduleClient({ consultations }: { consultations: Consultation[] }) {
  const t = useTranslations('SpecialistSchedule');
    ...
    <div className="min-h-screen bg-slate-50/50 p-8">
      ...
      {sortedDates.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 bg-white rounded-3xl border border-slate-100 shadow-sm">
          ...
          <h2 className="text-xl font-bold text-slate-900">{t('noAppointments')}</h2>
        ...
        <div className="space-y-8">
          {sortedDates.map(dateStr => {
            ...
            if (isToday(date)) dayLabel = t('today');
            if (isTomorrow(date)) dayLabel = t('tomorrow');
                          ...
                          <div>
                            ...
                            <div className="flex items-center gap-3 text-sm text-slate-500">
                              <span className="flex items-center gap-1">
                                <Video size={14} /> {t('videoConsultation')}
                              </span>
