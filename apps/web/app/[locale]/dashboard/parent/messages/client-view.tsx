...
export default function MessagesClient({ 
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(t('errorMessage', { message: error.message || t('sendFailed') }));
      ...
      <div className={`w-full md:w-80 border-e border-slate-200 flex flex-col ${selectedRecipientId ? 'hidden md:flex' : 'flex'}`}>
        ...
        <div className="p-4 border-b border-slate-100 space-y-4">
          ...
          <div className="relative">
            ...
            <input 
              type="text" 
              placeholder={activeTab === 'specialists' ? t('searchSpecialist') : t('searchMember')}
              value={searchQuery}
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0 text-start">
                  ...
                  <p className={`text-sm truncate ${selectedRecipientId === person.id ? 'text-medical-600' : 'text-slate-500'}`}>
                    {activeTab === 'specialists' 
                      ? person.specialist_details?.specialty || t('roleSpecialist')
                      : t('roleParent')}
                  </p>
                </div>
              </div>
      ...
      {selectedRecipientId ? (
        ...
        <div className="hidden md:flex flex-1 flex-col items-center justify-center bg-slate-50/50">
          <div className="text-center p-8">
            ...
            <h2 className="text-2xl font-bold text-slate-900 mb-2">{t('messagesTitle')}</h2>
