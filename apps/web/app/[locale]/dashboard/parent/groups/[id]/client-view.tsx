...
export default function GroupFeed({ 
  ...
  const handlePost = async (e: React.FormEvent) => {
    ...
    const tempPost = {
      ...
      author: {
        full_name: currentUser.user_metadata?.full_name || t('me'),
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="mt-4 pt-4 border-t border-slate-50 animate-in slide-in-from-top-2 duration-200">
                  <div className="space-y-4 mb-4">
                    {loadingComments && !post.comments && (
                      <div className="text-center text-slate-400 py-2">{t('loadingComments')}</div>
                    )}
                    ...
                    {post.comments?.length === 0 && (
                      <div className="text-center text-slate-400 text-sm py-2">{t('noComments')}</div>
                    )}
                  </div>
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm">
            <h3 className="font-bold text-slate-900 mb-4">{t('aboutGroup')}</h3>
            <p className="text-sm text-slate-500 mb-4">
              {group.description}
            </p>
            <div className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">{t('rules')}</div>
