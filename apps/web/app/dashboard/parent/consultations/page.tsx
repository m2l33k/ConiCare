  ...
  return (
    <div className="space-y-8">
      <header className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-slate-900">الاستشارات</h1>
          <p className="text-slate-500 mt-2">إدارة مواعيدك وجلسات الفيديو مع الأخصائيين</p>
        </div>
        <Link 
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        <div className="p-6 border-b border-slate-100 bg-slate-50/50">
          <h3 className="font-bold text-slate-900">المواعيد القادمة</h3>
        </div>
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  <h4 className="font-bold text-slate-900 text-lg mb-1">
                    استشارة مع {consultation.specialist?.full_name || 'Specialist'}
                  </h4>
              ...
              <div className="flex items-center gap-3">
                ...
                {consultation.meeting_link && consultation.status === 'scheduled' ? (
                  ...
                  <span className={`px-4 py-2 rounded-lg font-bold text-sm ${
                    ...
                    {consultation.status === 'completed' && <span className="flex items-center gap-1"><CheckCircle size={14} /> Completed</span>}
                    {consultation.status === 'cancelled' && <span className="flex items-center gap-1"><XCircle size={14} /> Cancelled</span>}
                    {consultation.status === 'scheduled' && 'Pending Link'}
                  </span>
                )}
              </div>
            </div>
