            ...
            {(patients || []).map((patient: any) => (
              <tr key={patient.id} className="hover:bg-slate-50 transition-colors group">
                ...
                <td className="px-6 py-4 text-slate-600">
                  {new Date().getFullYear() - new Date(patient.birth_date).getFullYear()} yrs
                </td>
                <td className="px-6 py-4">
                  <div className="text-sm font-medium text-slate-900">{patient.profiles?.full_name || 'Unknown'}</div>
