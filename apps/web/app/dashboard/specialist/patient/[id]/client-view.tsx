      ...
      <PanelGroup direction="horizontal" className="h-full">
        ...
        <Panel defaultSize={20} minSize={15} maxSize={25} className="bg-white border-r border-slate-200 flex flex-col">
          ...
          <div className="p-6 space-y-6 overflow-y-auto flex-1">
            <div>
              <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3">Guardian Contact</h3>
              <div className="flex items-start gap-3 text-sm text-slate-600">
                ...
                <div>
                  ...
                  <p className="text-xs">{child.profiles.email || 'No email provided'}</p>
                </div>
              </div>
            </div>

            <div>
              <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3">Clinical Diagnosis</h3>
              <div className="flex flex-wrap gap-2">
                {child.conditions && child.conditions.length > 0 ? (
                  ...
                  <p className="text-sm text-slate-400 italic">No conditions logged.</p>
                )}
              </div>
            </div>

            <div>
              <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3">Assessments History</h3>
          ...
          <div className={`relative bg-black transition-all duration-500 ease-in-out ${compareMode ? 'h-1/2' : 'h-3/5'} shrink-0 flex items-center justify-center overflow-hidden`}>
            ...
            <div className={`relative h-full ${compareMode ? 'w-1/2 border-r border-slate-700' : 'w-full'}`}>
               {currentAssessment ? (
                 ...
                 <div className="flex items-center justify-center h-full text-slate-500">No Assessment Selected</div>
               )}
               <div className="absolute top-4 left-4 bg-black/50 text-white px-3 py-1 rounded-full text-xs font-mono backdrop-blur-md">
                 {currentAssessment ? new Date(currentAssessment.created_at).toLocaleDateString() : 'Select Date'}
               </div>
            </div>
            ...
            {compareMode && (
              <div className="w-1/2 h-full relative">
                {comparisonAssessment ? (
                 ...
                 <div className="absolute top-4 left-4 bg-black/50 text-white px-3 py-1 rounded-full text-xs font-mono backdrop-blur-md">
                   {comparisonAssessment ? new Date(comparisonAssessment.created_at).toLocaleDateString() : 'Comparison'}
                 </div>
              </div>
            )}
          ...
          <div className="flex-1 flex flex-col overflow-hidden">
            ...
            <div className="flex-1 flex flex-col min-h-0 border-b border-slate-200">
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50/30">
                {notes.length === 0 && (
                  <p className="text-center text-slate-400 text-sm py-4">No notes logged yet.</p>
                )}
                {notes.map((note) => (
            ...
            <div className="h-1/3 flex flex-col bg-medical-50/30">
              ...
              <div className="p-4 overflow-y-auto">
                ...
                <div className="mt-4 space-y-2">
                  <h4 className="text-xs font-bold text-slate-400 uppercase">Active Tasks</h4>
                  {prescriptions.map(p => (
